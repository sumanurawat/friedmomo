...
//
// The desktop build already has a real filesystem at ~/Storyboarder, so users
// can back up or migrate by copying that folder. We stub these here so the
// facade exports the same surface in both modes — calling them in Electron
// isn't a crash, just a clear "not applicable".
// ---------------------------------------------------------------------------

export async function exportProjectBundle() {
  throw new Error(
    'Bundle export is only available in the web build. Desktop users can copy ~/Storyboarder directly.',
  );
}

export async function importProjectBundle() {
  throw new Error(
    'Bundle import is only available in the web build. Desktop users can copy files into ~/Storyboarder directly.',
  );
}
